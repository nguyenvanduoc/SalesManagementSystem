...
--              - Cập nhật NS_DonDatHang & NS_DonDatHangChiTiet
--              - Cập nhật BAN_ChungTuBanHang & BAN_ChungTuBanHang_ChiTiet
--              - Cập nhật KHO_PhieuXuat & KHO_PhieuXuat_ChiTiet
--              - Ghi lịch sử điều chỉnh DON_DieuChinhDonHang
--              - Ghi giao dịch kho KHO_GiaoDichKho nếu đã xuất kho
-- =======================================================
...
BEGIN
    ...
    -- 2. Parse chi tiết mới từ JSON (camelCase từ JavaScript validateAndSerialize)
    ...
    BEGIN TRY
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        VALUES
                            (CAST(GETDATE() AS DATE), @soDieuChinh, 2, 0, @IDKho, @spId,
                             0, @delta, @dgMoiOrCu, @dgMoiOrCu * @delta,
                             N'Xuất điều chỉnh tăng BH theo phiếu ' + @soDieuChinh, GETDATE(), @NguoiTao);
                    END
                    ELSE IF @delta < 0
                    BEGIN
                        ...
                        INSERT INTO KHO_GiaoDichKho
                            (NgayChungTu, SoChungTu, LoaiChungTu, IDChiTietKho, IDKho, IDSanPham,
                             SoLuongNhap, SoLuongXuat, DonGia, ThanhTien, DienGiai, NgayTao, NguoiTao)
                        VALUES
                            (CAST(GETDATE() AS DATE), @soDieuChinh, 1, 0, @IDKho, @spId,
                             @actualDelta, 0, @dgCuOrMoi, @dgCuOrMoi * @actualDelta,
                             N'Nhập điều chỉnh giảm BH theo phiếu ' + @soDieuChinh, GETDATE(), @NguoiTao);
                    END
                END
            END
        ...
        BEGIN
            ...
            INSERT INTO BAN_ChungTuBanHang_ChiTiet
                (IDChungTuBanHang, IDSanPham, STT, SoLuong, DonGia,
                 DonGiaBocXep, ThanhTienBocXep,
                 SoTienChietKhau, ChuongTrinhTichLuySale,
                 ThanhTienHang, ThanhTien, ThueGTGT, TienThue, TongSauThue, GhiChu)
            SELECT
                @invoiceId, IDSanPham, ROW_NUMBER() OVER(ORDER BY IDSanPham),
                SoLuong, DonGia,
                DonGiaBocXep, ThanhTienBocXep,
                SoTienChietKhau, ChuongTrinhTichLuySale,
                ThanhTienHang, ThanhTien, ThueGTGT, ThanhTienThue, ThanhTienSauThue, GhiChu
            FROM @ChiTietMoi;
        ...
        BEGIN
            ...
            SELECT
                @shipId, IDSanPham, ROW_NUMBER() OVER(ORDER BY IDSanPham),
                SoLuong, DonGia,
